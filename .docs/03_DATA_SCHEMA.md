...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `text` | `string[]` | ❌ | 순수 지문 (줄바꿈 단위 배열). 빈칸 패턴 없음 |
| `blanks` | `string[]` | ❌ | 빈칸 채우기 지문. `( ㉠ )`, `( )` 등 패턴 포함 |
| `underlines` | `number[]` | ❌ | `text[]` 기준으로 밑줄 표시할 줄의 인덱스 배열 |
| `list` | `ListItem[]` | ❌ | 기호 기반 항목 데이터 (ㄱ, ㄴ, ㉠ 등) |
| `media` | `ExamMedia` | ❌ | 이미지 또는 도표 정보 |
| `table` | `ExamTable` | ❌ | 표 데이터 |

> `text`와 `blanks`는 의미가 다르며 같은 view에 공존할 수 있다. `text`는 plain 렌더, `blanks`는 빈칸 기호 강조 렌더가 적용된다.

### ListItem (항목 리스트 한 항목)
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
...
### 2. 밑줄 지문 (text + underlines)
```json
{
  "question": "밑줄 친 A 선수의 입장과 관련된 사상으로 적절한 것은?",
  "view": {
    "text": [
...
```json
{
  ...
  "view": {
    ...
    "blanks": [
      "심부체온이 증가하여 산소-헤모글로빈 해리 곡선은 ( ㉠ )으로 이동한다."
    ]
  },
  "answer": [1]
}
